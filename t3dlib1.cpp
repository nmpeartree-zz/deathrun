...
int Zoom_Bit(BOB_PTR bob,               // bob to draw
             LPDIRECTDRAWSURFACE7 dest) // surface to draw the bob on
{
// draw a bob at the x,y defined in the BOB
// on the destination surface defined in dest
...
// fill in the destination rect
dest_rect.left   = 400-bob->x;
dest_rect.top    = 300-bob->y;
dest_rect.right  = 400+bob->x;
dest_rect.bottom = 300+bob->y;
...
int Draw_BOB16Scaled(BOB_PTR bob,             // bob to draw
             LPDIRECTDRAWSURFACE7 dest,int width, int height) // surface to draw the bob on
{
// draw a bob at the x,y defined in the BOB
// on the destination surface defined in dest
...
dest_rect.left   = bob->x;
dest_rect.top    = bob->y;
dest_rect.right  = bob->x+width;
dest_rect.bottom = bob->y+height;
...
int Collision_BOBS(BOB_PTR bob1, BOB_PTR bob2, int width1, int height1, int width2, int height2)
{
...
*/
 width1  = (width1>>1) - (width1>>3);
 height1 = (height1>>1) - (height1>>3);

 width2  = (width2>>1) - (width2>>3);
 height2 = (height2>>1) - (height2>>3);
...
int Scale_Polygon2Dy(POLYGON2D_PTR poly,int sy)
{
// this function scalesthe local coordinates of the polygon
...
/*
		AdjustWindowRectEx(&window_rect,GetWindowStyle(hwnd),
							GetMenu(hwnd) != 0,
...
*/
		DDraw_Init(SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_BPP,1);
		//lpddclipper = DDraw_Attach_Clipper(lpddsback,1,&window_rect);
/*
	if (FAILED(lpdd->SetCooperativeLevel(main_window_handle,DDSCL_NORMAL)))
       return(0);
